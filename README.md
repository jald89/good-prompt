...
La demo de la interfaz vive en `src/web` y usa Vite + TailwindCSS para mostrar el kit de componentes. Sigue estos pasos desde la raíz del repositorio:

1. **Instala dependencias** (Node.js ≥ 18):

   ```bash
   npm install
   ```

2. **Arranca el servidor de desarrollo** en modo UI:
...
- `npm run preview:web`: sirve el build ya generado para revisar cómo se verá en producción.

Si necesitas reiniciar el servidor, presiona `Ctrl + C` en la terminal y vuelve a ejecutar `npm run dev:web`.
